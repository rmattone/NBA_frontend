      ...
      <b-card class="mb-4">
        ...
        <div class="icon-grid">
          <div class="icon-box" v-for="(item, index) in iconLists" :key="index">
            <div class="overlay">
              <button class="btn btn-sm btn-soft-primary" v-b-tooltip.hover :title="iconTitle" @click="copyIcon(item.name)">
                {{ iconTitle }}
              </button>
            </div>
...
const onSearchUpdate = () => {
  router.push({ name: 'icons.outlined', query: { q: search.value } })
}
</script>
