<template>
  <div class="pb-4 bg-transparent">
    <h4>{{ $t('blog.sidebar.category') }}</h4>
  </div>
  <b-card no-body>
    <b-card-body class="card-thumbnail">
      <DataTable :data="BlogCommentsTable.data" :columns="BlogCommentsTable.columns" />
    </b-card-body>
  </b-card>
</template>
<script setup>
  ...
  columns: [
    {
      ...
      render: function (data) {
        return '  <div class="d-flex align-items-center"> <img class="rounded img-fluid avatar-65 me-3" src=' + data.img + ' alt="" loading="lazy" /><div className="media-support-info"><h5 class="iq-sub-label">' + data.name + '</h5><p class="mb-0">' + data.email + '</p></div></div>'
      }
    },
    {
      title: 'Contact',
