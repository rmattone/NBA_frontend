<template>
  <div class="d-flex justify-content-center gap-3">
    <img :src="`${userAvatar}`" class="img-fluid avatar-48 rounded-circle" alt="profile-image-1" loading="lazy" />
    <div class="d-flex align-items-center justify-content-between flex-grow-1">
      <div class="me-3">
        <h5>{{ userName }}</h5>
        <small class="text-ellipsis short-1">{{ userComment }}</small>
      </div>
      <img :src="`${postImg}`" class="img-fluid avatar-40 rounded object-contain" alt="profile-image" loading="lazy" v-if="image == 'true'" />
      <b-button variant="sm btn-icon btn-primary rounded-pill" v-if="addBtn == 'true'">
